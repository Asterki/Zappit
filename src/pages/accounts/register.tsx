...
import type { CheckUseBody } from "../../../shared/types/api";
...
const Register: NextPage<PageProps> = (props: PageProps): JSX.Element => {
    ...
    const [passwordStrengthVariant, setPasswordStrengthVariant] = React.useState("danger" as "success" | "warning" | "danger");

    const [emailError, setEmailError] = React.useState("" as keyof typeof LangPack.accounts.register.errors);
    const [passwordError, setPasswordError] = React.useState("" as keyof typeof LangPack.accounts.register.errors);
    ...
    const checkValues = async (event: React.MouseEvent): Promise<void | string> => {
        ...
        if (!parsedData.success && parsedData.error) {
            setEmailError(parsedData.error.errors[0].message as keyof typeof LangPack.accounts.register.errors);
