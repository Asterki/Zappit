...
router.post(
	...
	(req, res) => {
		...
		Users.findOne({ 'email.value': req.body.email }, async (err, user) => {
			...
			if (emailVerificationTokens == null) emailVerificationTokens = [];
			...
			await redisClient.set('emailVerificationTokens', JSON.stringify(emailVerificationTokens));
